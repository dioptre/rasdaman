...
public class Wcs2Servlet extends HttpServlet {
    ...
    public void init() throws ServletException {
        ...
        ConfigManager.getInstance(context.getRealPath(SETTINGS_PATH), context.
                getRealPath("/"));
        PropertyConfigurator.configure(context.getRealPath("/log4j.properties"));

        // Read servlet HTML usage message from disk
        try {
            usageMessage = FileUtils.readFileToString(new File(context.getRealPath(USAGE_FILE)));
        } catch (IOException e) {
    ...
    private void setServletURL(HttpServletRequest req) {
        if (ConfigManager.WCS2_SERVLET_URL == null) {
            System.out.println(req.getRequestURL().toString());
            ConfigManager.WCS2_SERVLET_URL = req.getRequestURL().toString();
        }
    }
