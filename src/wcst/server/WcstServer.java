...
public class WcstServer
{
	...
	public WcstServer(DbMetadataSource source) throws WCSException
	{
        ...
        if (source.getDataTypes().contains(cfg) == false)
            throw new WCSException("InvalidPropertyValue", "The following setting is not a valid datatype: " + cfg);
        cfg = ConfigManager.WCST_DEFAULT_INTERPOLATION;
        if (source.getInterpolationTypes().contains(cfg) == false)
            throw new WCSException("InvalidPropertyValue", "The following setting is not a valid interpolation method: " + cfg);
        cfg = ConfigManager.WCST_DEFAULT_NULL_RESISTANCE;
        if (source.getNullResistances().contains(cfg) == false)
            throw new WCSException("InvalidPropertyValue", "The following setting is not a valid null resistance: " + cfg);
	}
		...
		try
		{
			try
			{
				...
				if ( synchronous == true )
				{
                    ...
                    log("Marshalling with context: " + response.getClass().getPackage().getName());
                    ...
                    log("Done! User has the TransactionResponse result !");
				}
				else
			...
			catch (JAXBException e)
			{
				throw new WCSException("XmlStructuresError",
                        "Could not marshall/unmarshall XML structures.", e);
			}
			catch (DatatypeConfigurationException e)
			{
				throw new WCSException("XmlStructuresError",
                        "Could not build request acknowledgement. ", e);
			}
		}
